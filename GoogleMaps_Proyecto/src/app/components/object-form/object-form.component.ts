...
export class ObjectFormComponent implements OnInit {
  ...
  match(){
    let location1= localStorage.getItem('location1');
    let location2= localStorage.getItem('location2');

    
    const distance = google.maps.geometry.spherical.computeDistanceBetween( this.fundacionDonBoscoLatLng, this.fundacionDonBoscoLatLng);
